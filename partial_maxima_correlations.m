...
for i = 1:T_h;
    ...
    M_h(i+1,1) = sigma(i*h,Lambda,beta)*W_n(i,1);
end
...
hold on;
plot(pow(t,epsilon),pow(s*(Lambda/(Lambda-1)),epsilon),'Color','b','LineWidth',1.5);
hold on;
plot(pow(t,epsilon),pow(s*((Lambda-1)/Lambda),epsilon),'Color','m','LineWidth',1.5);
hold on;
plot(pow(t,epsilon),pow(-s*((Lambda-1)/Lambda),epsilon),'Color','m','LineWidth',1.5);
hold on;
plot(pow(t,epsilon),pow(-s*(Lambda/(Lambda-1)),epsilon),'Color','b','LineWidth',1.5);
...
h = legend('$\frac{s(t)\,X_t(\omega)}{\Sigma(t)}$','$M(t)/\Sigma(t)$',...
